...
def figMetaData(file_path):
    ...
    url = "http://www.google.com/maps/place/"+str(latitude)+","+str(longitude)
...
def pdfMetaData(file_path):
    ...
    if pdf_doc.isEncrypted:
        try:
            if pdf_doc.decrypt("PASSWORD_GOES_HERE") != 1: #PASSWORD: "banana"
    ...
    decryptedPDF = PdfFileReader('decrypted_output.pdf')
    decryptedPDF_info = decryptedPDF.getDocumentInfo()

    for data in dir(decryptedPDF_info):
        print(f"\t{data}")

if __name__ == "__main__":
    parser = optparse.OptionParser("Usage: python <script_name> -p <file>")
    parser.add_option("-p", dest="file", type="string", help="please provide full path to the document")
